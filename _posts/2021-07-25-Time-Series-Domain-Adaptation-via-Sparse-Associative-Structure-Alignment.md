---
title: Time Series Domain Adaptation via Sparse Associative Structure Alignment （域自适应）
...
自适应分段为了解决此问题，对于每个变量X都构造了多个具有不同时间长度的片段，可以从框架图中更加清晰的看出，并且为每个时间片段都分配一个独立的LSTM，即每个时间片段的数据x都作为输入变量，输入到独立的LSTM模型中，在这里对LSTM模型进行一下介绍，[见先验知识③](#jump)
