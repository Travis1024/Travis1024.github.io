...
1. 系统为什么要分层？
   希望专人干专事，职能分工更加明确。这样可以让代码耦合度降低，扩展性增强，组建的可复用性增强。

2. 软件的架构中，有一个著名的架构模式，MVC架构模式
   M（Model：数据/业务）     V（View：视图/展示）     C（Controller：控制器）
   
   C（是核心，是控制器，是司令官）
   M（处理业务/处理数据的一个秘书）
   V（负责页面展示的一个秘书）
   MVC：一个司令官，调度两个秘书去做就这件事。  

<img src="https://cdn.jsdelivr.net/gh/Travis1024/PicGo_image/202209181147207.png"/>

​    其中Model包含pojo（bean、domain），service，dao

3. 三层架构
   
   <img src="https://cdn.jsdelivr.net/gh/Travis1024/PicGo_image/202209181722836.png" style="zoom:40%"/>
   
   - **事务一定是在业务逻辑层进行控制的，一般是一个业务方法对应一个完整的事务。**

4. 三层架构和MVC的关系
   
   <img src="https://cdn.jsdelivr.net/gh/Travis1024/PicGo_image/202209181726327.png"/>
